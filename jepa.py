...
class JEPA(nn.Module):
    ...
    def criterion(self, info_dict: dict):
        ...
        # return last-step cost using concentric shell distance
        u = pred_emb[..., -1:, :]
        v = goal_emb[..., -1:, :].detach()
        dist_full = poincare_distance(u, v)

        # Coarse representation: project both to a smaller inner shell of the Poincaré ball
        u_coarse = u * 0.1
        v_coarse = v * 0.1
        dist_coarse = poincare_distance(u_coarse, v_coarse)

        # Dynamic weighting based on full distance
        w = torch.exp(-dist_full)
        dist_concentric = (1.0 - w) * dist_coarse + w * dist_full
        cost = dist_concentric.pow(2).sum(dim=tuple(range(2, dist_concentric.ndim)))
